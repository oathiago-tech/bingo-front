    ...
    return (
        <div className="flex min-h-screen bg-slate-950 text-white overflow-hidden font-sans">
            ...
            <div className="fixed top-4 left-4 z-40">
                <div
                    className="bg-slate-900/80 backdrop-blur-md px-6 py-2 rounded-2xl border border-slate-800 shadow-2xl">
                    <span className="text-4xl font-black text-yellow-500 font-mono tracking-widest">{currentTime}</span>
                </div>
            </div>
            {showStartMessage && (
                <div
                    className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-yellow-500 text-black px-16 py-12 rounded-[3rem] font-black text-4xl animate-bounce border-[10px] border-white shadow-[0_0_100px_rgba(234,179,8,0.6)]">
            ...
            {currentBall && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/10 backdrop-blur-sm">
                    <div className="text-center animate-bounce">
                        <div
                            className="w-80 h-80 bg-white text-slate-950 rounded-full flex items-center justify-center text-[10rem] font-black border-[12px] border-slate-200 shadow-2xl">
                            {currentBall}
                        </div>
                    </div>
                </div>
            ...
            <div className="w-80 bg-slate-900 border-r border-slate-800 p-6 flex flex-col h-screen shadow-5xl mt-20">
                <h2 className="text-xl font-black text-yellow-500 mb-6 uppercase italic tracking-widest border-b border-slate-800 pb-4 text-center animate-pulse">📅
                    Sorteios de Hoje</h2>
                <div className="flex-1 space-y-3 overflow-y-auto pr-2 custom-scrollbar">
                    {dailyRaffles.length > 0 ? dailyRaffles
                            .map((r, i) => {
                                const raffleTime = new Date(r.raffleDate);
                                const raffleTimeString = raffleTime.toLocaleTimeString([], {
                                    hour: '2-digit',
                                    minute: '2-digit'
                                });
                                ...
                                return (
                                    <div key={i} className={`p-4 rounded-2xl border transition-all duration-500 flex justify-between items-center
                                        ...
                                        <span
                                            className={`font-black text-lg ${isActive ? 'text-black' : 'text-green-500'}`}>
                                            R$ {r.value.toFixed(2)}
                                        </span>
                                    </div>
                                );
            ...
            <div className="flex-1 flex flex-col items-center p-10 space-y-10 overflow-y-auto mt-12">
                ...
                <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl text-center">
                    <div className="flex items-center justify-center space-x-3 mb-2">
                        <div
                            className={`w-4 h-4 rounded-full ${isStarted ? 'bg-green-500 animate-pulse' : 'bg-red-500 animate-pulse'}`}></div>
                        <span
                            className="text-2xl font-bold uppercase animate-pulse">{isStarted ? 'Rodada Em Andamento' : 'Aguardando próxima rodada'}</span>
                    </div>
                    <p className="text-slate-500 text-sm font-black tracking-widest">PRÓXIMA
                        RODADA: {nextRaffleTime}</p>
                </div>
                {winners.length > 0 && (
                    <div
                        className="w-full max-w-5xl bg-red-600 p-10 rounded-[3rem] border-8 border-white text-center animate-pulse shadow-[0_0_80px_rgba(220,38,38,0.6)]">
                        <h2 className="text-3xl font-black italic mb-4 uppercase animate-pulse">RINGO!</h2>
                        {winners.map((w, i) => <div key={i}
                                                    className="text-2xl font-extrabold text-white drop-shadow-lg animate-pulse">- {w.storeName}</div>)}
                    </div>
                )}
            ...
            <div className="w-96 bg-slate-900 border-l border-slate-800 p-8 flex flex-col h-screen shadow-2xl">
                <h2 className="text-2xl font-black text-yellow-500 mb-8 uppercase italic tracking-widest border-b border-slate-800 pb-4 text-center animate-pulse">🏆
                    Prêmios do Dia</h2>
                            ...
                            <div key={i} className="bg-slate-800/40 p-5 rounded-3xl border border-slate-700">
                                ...
                                <div className="flex justify-between items-end mt-2">
                                    <p className="text-xs text-slate-500 font-mono font-bold">
                                        HORÁRIO: {new Date(w.raffleDate).toLocaleTimeString([], {
                                        hour: '2-digit',
                                        minute: '2-digit'
                                    })}
                                    </p>
